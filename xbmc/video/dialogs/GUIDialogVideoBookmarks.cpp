      ...
      else if (m_viewControl.HasControl(iControl))  // list/thumb control
      {
        ...
        if (iAction == ACTION_DELETE_ITEM)
        {
          if( (unsigned)iItem < m_bookmarks.size() )
          {
            ...
            if (g_application.CurrentFileItem().HasProperty("original_path"))
              path = g_application.CurrentFileItem().GetProperty("original_orginal_path").asString();
...
void CGUIDialogVideoBookmarks::Update()
{
  ...
  if (g_application.CurrentFileItem().HasProperty("original_path"))
    path = g_application.CurrentFileItem().GetProperty("original_path").asString();
  CVideoDatabase videoDatabase;
  videoDatabase.Open();
